function [sn_data_pos_filt, sn_data_neg_filt, nf_data_filt, fs_new]= get_filtered_ffr(snrVar, allfiles, data_dir, all_snrs, remove_artifact)
...
nf_data_filt= filtfilt(bpFilt, nf_data.*rampVector);

%
BaseAmp= 1e-6;
sn_data_pos_filt= helper.gen_resample(sn_data_pos_filt, fs_org, fs_new)/temp.data.AD_Data.Gain/BaseAmp; % Make amp re. 1uV
sn_data_neg_filt= helper.gen_resample(sn_data_neg_filt, fs_org, fs_new)/temp.data.AD_Data.Gain/BaseAmp;
nf_data_filt= helper.gen_resample(nf_data_filt, fs_org, fs_new)/temp.data.AD_Data.Gain/BaseAmp;
end