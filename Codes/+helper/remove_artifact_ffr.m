...
freq_out= helper.return_closest_max(f, Fval, fTest, ceil(2/(fs/nfft)), x_output_spread); % ceil(1/(fs/nfft)): To check in a 1 Hz range
...
if plotVar
    hold on;
    %     yrange=32;
    helper.plot_dpss_psd(in_data, fs_data, 'nw', params.tapers(1));
    helper.plot_dpss_psd(out_data, fs_data, 'nw', params.tapers(1));
    xlim([25 fMax+50]);
