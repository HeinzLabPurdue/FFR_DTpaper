...
for chinVar= 1:length(nhInds)
    ...
    % tfs 
    x= helper.dbspl(sqrt(pool_lf_power_audio(curInd, :)));
    y= db(sqrt(pool_tfs_power_ffr(curInd, :)));
    mdl2= fitlm(x, y);
    nh_slopes.tfs(chinVar)= mdl2.Coefficients.Estimate(2);
    
    % env    
    x= helper.dbspl(sqrt(pool_hf_power_audio(curInd, :)));
...
fprintf('Improvement: %.1f\n', 100*(std(nh_slopes.tfs)-std(nh_slopes.ratio))/std(nh_slopes.tfs))

fprintf('---------HI---------\n');
[std(hi_slopes.ratio) std(hi_slopes.tfs) std(hi_slopes.env)]
fprintf('Improvement: %.1f\n', 100*(std(hi_slopes.tfs)-std(hi_slopes.ratio))/std(hi_slopes.tfs))
